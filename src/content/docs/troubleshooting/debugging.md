...
- **IMPORTANT**: Set FTDI adapter to 3.3V mode
- Pin connections:
  - FTDI TX → MFM RX
  - FTDI RX → MFM TX
  - FTDI GND → MFM GND
  - FTDI VCC → MFM VCC (powers device at 3.3V)
  
:::tip[Power During Debug]
The FTDI adapter can power the device during debugging. Set the adapter to 3.3V mode and connect VCC. This eliminates the need for batteries during development. See [Powering the Device](/development/development-guide/#powering-the-device) for details.
:::
