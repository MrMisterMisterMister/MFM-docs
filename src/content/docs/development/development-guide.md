...
The Multiflexmeter can be powered in two ways:

#### 1. Battery Power (Field Deployment)
- Standard operation uses 2x AA batteries connected to the battery connector
- Operating voltage: 3.0V - 3.3V (2x 1.5V batteries)
- The device includes battery monitoring and low-power modes

#### 2. External Power (Development/Testing)

For development and testing without batteries, you can power the device through the **FTDI connector**:
...
1. **Via FTDI Adapter** (easiest for debugging):
   - Connect FTDI adapter to the FTDI header
   - The FTDI VCC pin provides 3.3V or 5V power
   - **Set FTDI adapter to 3.3V mode** (most FTDI adapters have a jumper)
   - This powers the device while also providing serial debug output

2. **Via External Power Supply**:
   - Connect regulated 3.3V power supply to VCC and GND pins on FTDI header
   - Voltage range: **3.0V - 3.6V** (3.3V recommended)
   - Current: ~100mA during transmission, <1mA sleep mode
   - **DO NOT exceed 3.6V** - may damage components

:::warning[Voltage Warning]
The Multiflexmeter operates at **3.3V logic levels**. Do not connect 5V power directly to VCC when batteries are removed. Always use 3.3V regulated power or set your FTDI adapter to 3.3V mode.
:::

:::tip[Development Setup]
For convenient development:
1. Remove batteries from battery holder
2. Connect FTDI adapter set to 3.3V mode
3. This provides both power and serial debug output
4. No need for separate power supply
:::
...
1. **Connect FTDI Adapter**
   ...
   - RX (FTDI) → TX (MFM)
   - GND → GND
   - VCC (FTDI) → VCC (MFM) - **Set adapter to 3.3V mode** to power the device
   - Or use separate power supply (see [Powering the Device](#powering-the-device))
